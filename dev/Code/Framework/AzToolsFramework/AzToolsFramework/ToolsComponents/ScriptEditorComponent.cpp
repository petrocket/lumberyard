    ...
    namespace Components
    {
        ...
        void ScriptEditorComponent::Activate()
        {
            ...
            if (m_scriptAsset.GetId().IsValid())
            {
                ...
                m_scriptAsset = AZ::Data::AssetManager::Instance().GetAsset<AZ::ScriptAsset>(m_scriptAsset.GetId());
                SetScript(m_scriptAsset);
        ...
        void ScriptEditorComponent::LoadProperties()
        {
            ...
            bool success = false;
            EBUS_EVENT_RESULT(success, AZ::ScriptSystemRequestBus, Load, GetScript(), context->GetId());
            if (!success)
            {
                return;
            }
        ...
        {
            AZ::Data::Asset<AZ::ScriptAsset> asset = AZ::Data::AssetManager::Instance().GetAsset<AZ::ScriptAsset>(assetId, false);
            if (asset)
            {
                SetScript(asset);
                ScriptHasChanged();
